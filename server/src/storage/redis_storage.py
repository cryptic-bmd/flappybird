...
from src.storage.base_storage import StateStorageBase, StateDataContext
...
class StateRedisStorage(StateStorageBase):
    ...
    :param prefix: Prefix for keys, default is "game".
    ...
    def get_data(self, user_id: Optional[int] = None) -> dict:
        _key = self._get_key(
            self.prefix,
            self.separator,
            user_id=user_id,
    ...
    def set_data(
        ...
        def set_data_action(pipe: Pipeline):
            pipe.multi()
    ...
    def reset_data(
        ...
        def reset_data_action(pipe: Pipeline):
            pipe.multi()
            if pipe.exists(_key):
                pipe.hset(_key, "data", "{}")
            else:
    ...
    def set_state(
        ...
        def set_state_action(pipe: Pipeline):
            pipe.multi()
    ...
    def delete_state(self, user_id: Optional[int] = None) -> bool:
        _key = self._get_key(
            self.prefix,
            self.separator,
            user_id=user_id,
    ...
    def get_interactive_data(
        ...
        return StateDataContext(self, user_id=user_id)
    ...
    def save(
        ...
        def save_action(pipe: Pipeline):
            pipe.multi()
            if pipe.exists(_key):
                pipe.hset(_key, "data", json.dumps(data))
            else:
                pipe.hset(_key, mapping={"state": None, "data": json.dumps(data)})

        self.redis.transaction(save_action, _key)

    def migrate_format(self, bot_id: int, prefix: Optional[str] = "game_"):
        """
        ...
        :param prefix: Prefix for keys, default is "game_"(old default value)
        :type prefix: Optional[str]
        """
